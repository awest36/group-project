...
function cloneRepo() {
	...
	cmd.get(
		'rm -rf group-project && git clone https://github.com/awest36/group-project.git',
...
function createRemoteTempFolder() {
	return ssh.execCommand('rm -rf group-project-temp && mkdir group-project-temp', { cwd:'/home/ubuntu' })
}
...
function updateRemoteApp() {
	return ssh.execCommand('cp -r group-project-temp/* group-project/ && rm -rf group-project-temp/*', { cwd:'/home/ubuntu' })
}
...
function sshConnect() {
	...
	ssh.connect({
		...
		host: '52.14.184.156',
		username: 'ubuntu',
		privateKey: 'g-key.pem'
	})
	.then(function() {
		console.log("SSH Connection established.");

		// Create "group-project-temp" directory on remote server
		console.log("Creating `group-project-temp` folder.");
